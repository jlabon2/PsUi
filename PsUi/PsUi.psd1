...
- New-UiTree: dotted property paths (e.g. 'Manager.Name') now walk into child objects instead of being read as one literal property name (also IdProperty, PathProperty, DisplayProperty)
- New-UiTree: path-mode parent nodes now carry a stand-in tag so click handlers always have something to read; gets promoted to the real item if a matching piped item arrives later
