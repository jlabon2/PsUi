function New-UiButtonCard {
    ...
    .PARAMETER ResultActions
        Actions offered against selected rows in the output window's results grid. Pass a
        { New-UiResultAction ... } definition block, an array of New-UiResultAction output,
        or the legacy hashtable array. See New-UiButton -ResultActions for the entry
        details.
    ...
    # Normalized here rather than in the inner New-UiButton so a bad definition names this function.
    if ($null -ne $ResultActions) {
        $ResultActions = [hashtable[]](ConvertTo-UiDefinitionArray -InputObject $ResultActions -ParameterName '-ResultActions' -CallerName 'New-UiButtonCard')
    }
