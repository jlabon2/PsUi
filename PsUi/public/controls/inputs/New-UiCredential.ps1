function New-UiCredential {
    ...
    .DESCRIPTION
        ...
        is a standard text input, while the password field is masked and carries the
        same eye button as New-UiInput -Password (hold to reveal, -NoPeek turns it
        off). In -Action blocks, the hydrated variable contains a PSCredential
        object.
    .PARAMETER Variable
    ...
    .PARAMETER NoPeek
        Hide the peek button on the password field. By default an eye icon next to the
        field reveals the password while held.
    ...
    .PARAMETER ClearIfDisabled
        Accepted alongside -EnabledWhen, but the clear currently never fires. The credential
        fields keep their values when disabled either way.
    ...
    # Same hold to reveal treatment as New-UiInput -Password. The Grid the helper returns carries the eye button.
    $peekResult = New-PasswordInputWithPeek -NoPeek:$NoPeek
    $passBox    = $peekResult.PasswordBox
    [void]$passContainer.Children.Add($peekResult.Container)
