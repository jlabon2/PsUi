function New-UiSlider {
    ...
    .DESCRIPTION
        Renders a horizontal or vertical slider with optional tick marks and a live value
        label. -IsSnapToTick makes the thumb land on ticks. The label takes a format string
        (percentage, decimal) and -EnabledWhen gates the control like any other input.
    .PARAMETER Variable
    ...
    .PARAMETER ClearIfDisabled
        When used with -EnabledWhen, resets the slider to its Minimum when the control
        becomes disabled.
