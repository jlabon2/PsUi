function New-ChartPieSlice {
    ...
    # Sweep angle over 360 degrees (one data point) is a full circle, which is drawn as an EllipseGeometry rather than a path
    if ($SweepAngle -ge 360) {
        $geometry = [System.Windows.Media.EllipseGeometry]::new([System.Windows.Point]::new($CenterX, $CenterY), $Radius, $Radius)
    }
    else {
        ...
        [void]$geometry.Figures.Add($pathFigure)
    }
