        ...
        private void ExecuteWithDedicatedRunspace(ScriptBlock script, Hashtable parameters, IDictionary variablesToDefine, IDictionary functionsToDefine, IEnumerable<string> modulesToLoad, bool debugEnabled)
        {
            ...
            RunOnBackgroundThread(delegate
            {
                ...
                try
                {
                    ...
                    {
                        psSetup.Runspace = rs;
                        psSetup.AddScript("[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $global:OutputEncoding = [System.Text.Encoding]::UTF8");
                        
                        // Enable debug output if requested
                        if (debugEnabled)
                        {
                            psSetup.AddScript("$Global:DebugPreference = 'Continue'");
                        }
                        ...
                        // Override Clear-Host to call our provider
                        psSetup.AddScript(@"
                        ...
                        // Override Write-Host using alias to ensure it takes precedence over cmdlet
                        //
                        // We are... sorta hijacking Write-Host. This proxy mimics its parameter signature.
                        // If Microsoft adds new params in a future PS version, this breaks, but why would they?
                        //  Binary modules that bypass PSHost and write directly to Console.WriteLine wont get intercepted
                        // either - that output just vanishes into the void. Not much we can do about that.
                        psSetup.AddScript(@"
                        ...
                        // Override Out-Host to route output through AsyncExecutor
                        psSetup.AddScript(@"
                        ...
                        // Load linked modules (quotes escaped)
                        if (modulesToLoad != null)
                        {
                            foreach (string mod in modulesToLoad)
                            {
                                if (!string.IsNullOrWhiteSpace(mod))
                                {
                                    ...
                                    psSetup.AddScript(string.Format("Import-Module '{0}' -ErrorAction SilentlyContinue", escapedMod));
                                }
                            }
                        }

                        // Wire up user functions (names validated first)
                        if (functionsToDefine != null)
                        {
                            foreach (DictionaryEntry kvp in functionsToDefine)
                            {
                                try 
                                {
                                    ...
                                    // Validate function name with whitelist pattern
                                    if (!Constants.IsValidIdentifier(name)) continue;
                                    
                                    string definition = kvp.Value.ToString();
                                    psSetup.AddScript(string.Format("function Global:{0} {{ {1} }}", name, definition));
                                }
                                catch (Exception ex)
                                {
                                    if (OnError != null) RaiseOnError(string.Format("Failed to link function '{0}': {1}", kvp.Key, ex.Message));
                                }
                            }
                        }

                        // Inject private functions from ModuleContext
                        ...
                        if (privateFuncs != null && privateFuncs.Count > 0)
                        {
                            foreach (DictionaryEntry kvp in privateFuncs)
                            {
                                ...
                                catch (Exception ex) { Debug.WriteLine("AsyncExecutor Dedicated Runspace Function Error: " + ex.Message); }
                            }
                        }
